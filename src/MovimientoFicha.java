public class MovimientoFicha {
    ...
    public static String[] calcularMovimientosPeon(int[] posicionInicial, boolean esBlanco) {
        String[] movimientos = new String[1];
        int fila = posicionInicial[0];
        int columna = posicionInicial[1];
        ...
        if (esBlanco) {
            if (fila > 0) {
                movimientos[0] = (char) ('A' + columna) + Integer.toString(8 - (fila - 1));
            }
        } else {
            if (fila < 7) {
                movimientos[0] = (char) ('A' + columna) + Integer.toString(8 - (fila + 1));
    ...
    public static String[] calcularMovimientos(String tipoFicha, int[] posicionInicial, boolean esBlanco) {
        ...
        switch (tipoFicha.toLowerCase()) {
            ...
            case "peón":
                return calcularMovimientosPeon(posicionInicial, esBlanco);
            default:
